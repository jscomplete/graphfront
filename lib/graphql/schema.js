...
const generateFields = (columns) => {
  ...
  columns.forEach(column => {
    if (!column.column_name.match(/^(created_at|updated_at)$/)) { // No input for these
      filteredColumns.push(column);
    }

    if (column.constraint_type === 'FOREIGN KEY') {
      relationFields.push(
        `${toSingular(column.related_table_name)}: ${toModelName(column.related_table_name)}`
      );
    }
    ...
    if (column.constraint_type === 'UNIQUE' || column.constraint_type === 'PRIMARY KEY') {
  ...
  return {
    viewerFields: columns
      ...
      .map(column =>
        `${toCamelCase(column.column_name)}: ${graphType(column.data_type, column.is_nullable === 'NO')}`
      ).join('\n'),
    inputFields: filteredColumns
      .filter(column => column.column_name !== primaryKey)
      .map(column =>
        `${toCamelCase(column.column_name)}: ${graphType(column.data_type, column.is_nullable === 'NO' && column.column_default === null)}`
      ).join('\n'),
    optionalFields: filteredColumns
      .map(column =>
        `${toCamelCase(column.column_name)}: ${graphType(column.data_type, false)}`
      ).join('\n'),
...
exports.generateSchema = (db, apiKeyValidator) => (dbSchema) => {
  ...
  return db(dbSchema).getTablesInfo().then(data => {
